...
export function calculateRenewalCharge(sub: SubscriptionResponse): number {
  ...
  const taxRate = 0.10;
  const amountInDollars = sub.amount / 100; // PayFlow v1.1 amount is in cents\n  const total = amountInDollars * (1 + taxRate);
  return total;
}
...
export function formatCustomerNotification(sub: SubscriptionResponse): string {
  ...
  const emailLower = (sub.customer.email || "noreply@example.com").toLowerCase();
